...
import { useState, useEffect } from "react";
    ...
    ar: {
        translation: {
                ...
                "Edit": "تعديل",
                "Delete": "حذف",
                "Are you sure you want to delete the Brand?": "هل أنت متأكد أنك تريد حذف العلامة؟",
                "No brands available": "لا يوجد علامات تجارية متاحة",
                'Active': 'نشط',
                "Inactive":'غير نشط',
        },
    },
...
export default function Index({ auth, brands, queryParams = null, success }) {
 ...
 useEffect(() => {
    ...
    const confirmationMessage = t("Are you sure you want to delete the Brand?");
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold leading-tight dark:text-gray-200">
            {t("Brands")}
              ...
              {auth.user.permissions.includes("create-brand") && (
        ...
        <div className="mx-auto sm:px-6 lg:px-8">
          ...
          <div className="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
            <div className="p-6 text-gray-900 dark:text-gray-100">
              <div className="overflow-auto">
                <table className="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400">
                  ...
                  <thead className="text-xs text-gray-700 uppercase border-b-2 border-gray-500 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    ...
                    <tbody>
                        {brands && brands.data.length > 0 ? ( brands.data.map((brand) => (
                        <tr
                            className="bg-white border-b dark:bg-gray-800 dark:border-gray-700"
                            key={brand.id}
                        >
                            <td className="px-3 py-2">{brand.id}</td>
                            <th className="px-3 py-2 text-nowrap">{brand.name}</th>
                           <th className="px-3 py-2 text-center text-nowrap">
                            <span
                                className={`inline-block px-2 py-1 rounded-full text-sm font-semibold ${
                                brand.is_active ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
                                }`}
                            >
                            ...
                            <td className="px-3 py-2 text-nowrap">
                            {/* Check if the user has permission to update the brand */}
                            {auth.user.permissions.includes("update-brand") && (
                                <Link
                            ...
                            {/* Check if the user has permission to delete the brand */}
                            {auth.user.permissions.includes("delete-brand") && (
                                <button
              ...
              {brands && <Pagination links={brands.meta.links} />}
            </div>
          </div>
        </div>
      </div>
